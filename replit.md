...
- **Per-post Open Graph image**: optional `image` frontmatter field. Accepts an absolute http(s) URL or a site path (e.g. `/static/og/my-post.png`). When set, `og:image` and `twitter:image` on `/blog/<slug>` use it. Resolved by `_blog_image_url()`. The blog list (`/blog`) shows the image as a small thumbnail on each post card when set. Recommended size 1200×630.
- **Auto-generated preview image**: when a post has no `image` frontmatter, the system serves a 1200×630 dark/green branded PNG at the stable URL `/blog/<slug>/og.png` (route `blog_post_og_image`). The image renders the post title (auto-shrinks for long titles, wraps to ≤4 lines), the site brand + logo (`static/favicon-logo.png`), the post date, and the site URL. `og:image` and `twitter:image` fall back to this URL (instead of the global `/static/og-image.png`) when no custom image is provided. Generation uses Pillow (`_generate_blog_og_image()`); results are cached on disk under `static/og/auto/<slug>.png` with a sidecar `<slug>.meta` SHA-256 fingerprint of the title — regenerated automatically when the title changes. If a post has a custom `image`, the route 302-redirects there instead. Drafts respect admin visibility (404 for non-admins). Dependency: `Pillow` (in `requirements.txt`).
